...
**Business model.** Heirloom charges nothing today, and no fee logic exists in the contract or the enclave. The proposal is free vault creation, a small flat fee to seal, and **0.3% of the estate at distribution, capped** — one more line in the TEE-signed distribution, so it is verified on-chain and cannot be hidden. Nothing is charged if the switch never fires, and since Heirloom can never take custody it can never charge a custody fee. Full detail, including what mainnet actually requires, is in [Business model & roadmap](ROADMAP.md).

**Immediate (days).** Move the encrypted will off-chain with only the commitment on-chain; replace the single regular key with a `SignerListSet` quorum so the enclave alone cannot sign; hardware-backed attestation on a Confidential Space VM.
