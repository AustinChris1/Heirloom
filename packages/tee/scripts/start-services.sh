...
if [[ "$USE_LOCAL" == "false" ]]; then
    ...
    if [[ -z "${REGISTRY:-}" ]]; then
        if ! docker image inspect local/tee-proxy >/dev/null 2>&1; then
            ...
            # Forward the refs from .env. Without these the Dockerfile defaults
            # (tee-proxy v0.0.18 / tee-node v0.0.21) win, and those sit below the
            # floor the Coston2 redeploy requires — data providers reject every
            # vote from an older node, so the queue silently stays empty.
            docker build -f "$PROXY_DOCKERFILE" -t local/tee-proxy \
                --build-arg TEE_PROXY_VERSION="${TEE_PROXY_VERSION:-develop}" \
                --build-arg TEE_NODE_VERSION="${TEE_NODE_VERSION:-develop}" \
                "$PROJECT_DIR/proxy" || die "Failed to build tee-proxy image"
