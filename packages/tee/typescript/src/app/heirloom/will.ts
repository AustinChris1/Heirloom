 ...
 *
 * Ported to viem to match the scaffold's dependency set: a smaller image builds
 * faster and keeps the attested code hash simpler to reproduce.
 */
...
export function parseWill(raw: unknown): Will {
  ...
  assert(
    typeof obj.estateAccount === "string" && isValidClassicAddress(obj.estateAccount),
 ...
 */
export function willCommitment(will: Will): `0x${string}` {
  const encoded = encodeAbiParameters(
    ...
    [
      ...
      will.bequests.map((b) => [
        b.beneficiary,
        (b.flareRecipient ?? ZERO_ADDRESS) as `0x${string}`,
        kindToEnum(b.kind),
        BigInt(b.amount),
      ]) as readonly (readonly [string, `0x${string}`, number, bigint])[],
    ] as never,
  );
...
export function standardAddressHash(address: string): `0x${string}` {
